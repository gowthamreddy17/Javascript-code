...
const startLogoutTimer = function () {
  ...
  tick(); //to call immediately at starting of timer after that setInterval will take charge and updates time by calling callback function for every 1 second.
...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentAccount?.pin === Number(inputLoginPin.value)) {
    ...
    // const now = new Date();
    // const day = `${now.getDate()}`.padStart(2, 0); //Here pad start used to display like 08 instead of 8. i.e, string should two characters if not 0 will be added.
    ...
    //Using Internationalization (name Space) API
    const now = new Date();
...
btnTransfer.addEventListener('click', function (e) {
  ...
  if (
    ...
    //Reset timer if tranfer money (inactive to active state of account)
    clearInterval(timer);
    timer = startLogoutTimer();
  }
});
...
btnLoan.addEventListener('click', function (e) {
  ...
  if (amount > 0 && currentAccount.movements.some(mov => mov >= amount * 0.1)) {
    setTimeout(function () {
      ...
      //Reset timer if request loan (inactive to active state of account)
      clearInterval(timer);
      timer = startLogoutTimer();
    }, 2500);
  }
...
const options = {
  style: 'currency', //style:'unit', //style:'percent', //
  //  unit:'mile-per-hour'
  unit: 'celsius',
  currency: 'EUR',
  // useGrouping:false,
