...
function processData(DataS, result) {
  ...
  for (const [roomid, seats] of Object.entries(DataS)) {
    ...
    // console.log(roomid, seats.seating);
  }
  return DataS;
}
...
//   for (const [roomid, seats] of Object.entries(data)) {
//     let roomData = {}; // Dictionary to store seat mappings for current room
//     for (let i = 0; i < seats.length; i++) {
//       for (let j = 0; j < seats[i].length; j++) {
//         roomData[`${i}-${j}-0`] = seats[i][j]; // Store seat ID in "row-column-layer" format
//       }
//     }
//     result[roomid] = roomData; // Store roomData in result with roomid as key
//   }
...
//             let seatData = {};
//             for (let j = 0; j < seats[i].length; j++) {
//                 seatData[`${j+1}:${i+1}`] = seats[i][j];
//             }
//             roomData.push(seatData);
//         }
//         result[roomid] = roomData;
//     }
//     console.log(result);
